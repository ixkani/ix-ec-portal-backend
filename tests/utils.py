...
class TestUtils(object):
    ...
    def _post_with_args(client, string, args, data):
        response = client.post(reverse(string, args=[args]), data, format='json',
                               secure=TestConstants.SECURE_CONNECTION, HTTP_HOST=TestConstants.HOST_URL)
        return response.status_code, response.data
    ...
    def _get_with_args(client, string, args):
        if type(args) is list:
            response = client.get(reverse(string, args=args), secure=TestConstants.SECURE_CONNECTION, format='json',
                                  HTTP_HOST=TestConstants.HOST_URL)
    ...
    def _put_with_args(client, string, args, data):
        if type(args) is list:
            response = client.put(reverse(string, args=args), data, secure=TestConstants.SECURE_CONNECTION,
                                  format='json', HTTP_HOST=TestConstants.HOST_URL)
        else:
            response = client.put(reverse(string, args=[args]), data, secure=TestConstants.SECURE_CONNECTION,
                                  format='json', HTTP_HOST=TestConstants.HOST_URL)
        return response.status_code, response.data
